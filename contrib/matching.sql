...
BEGIN
    ...
    FOR row IN EXECUTE 'select geometryType(GeometryFromText('''||ST_AsText(line)||''', '||srid||')) as type' LOOP
    END LOOP;
...
BEGIN
    ...
    FOR row IN EXECUTE 'select * from match_line('''||quote_ident(tbl)||''', GeometryFromText('''||ST_AsText(line)||''', '||srid||'), '
