...
BEGIN
    ...
    SELECT
        ...
        ST_Distance(the_geom,ST_GeometryFromText( ST_AsText(p), _srid)) AS d, id, the_geom

    INTO _r FROM vertices_tmp WHERE

        the_geom && ST_Expand(ST_GeometryFromText(ST_AsText(p), _srid), tolerance ) AND ST_Distance(the_geom, ST_GeometryFromText(ST_AsText(p), _srid)) < tolerance
